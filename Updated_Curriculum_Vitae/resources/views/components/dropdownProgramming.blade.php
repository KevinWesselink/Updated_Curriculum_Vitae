    ...
    <nav class="hidden space-x-1 md:flex">
        <div class="relative" x-data="{ open: false }">
            <button @click="open = ! open" type="button"
                ...
                aria-expanded="false">
                <span class="w-48">
                    <div class="text-teal-600">{{ $programming->languageName }}</div>
                    <div class="text-gray-500 text-xs p-1">{{ $programming->startedWith }} - {{ $programming->workedWithUntil }}</div>
                </span>
            ...
            <div x-show="open" x-transition:enter="transition ease-out duration-200"
                ...
                <div class="overflow-hidden rounded-lg shadow-lg ring-1 ring-black ring-opacity-5">
                    <div class="relative grid gap-6 bg-white px-2 py-2 sm:gap-2 sm:p-2">
                        <h3 class="text-2xl">{{ $programming->languageName }}</h3>
